...
fn u8_as_u16(data: &[u8]) -> IResult<&[u8], u16> {
    map(u8, |x| x as u16).parse(data)
}

fn i8_as_i16(data: &[u8]) -> IResult<&[u8], i16> {
    map(i8, |x| x as i16).parse(data)
}
...
impl Opcode {
    ...
    fn parse_opcode<'a>(opcode: u8, offset: usize, data: &'a [u8], constant_pool: &BTreeMap<u16, ConstantPoolItem>) -> IResult<&'a [u8], Self> {
        match opcode {
            ...
            0x19 => map(u8_as_u16, Opcode::Aload).parse(data),
            ...
            0x3a => map(u8_as_u16, Opcode::Astore).parse(data),
            ...
            0x18 => map(u8_as_u16, Opcode::Dload).parse(data),
            ...
            0x39 => map(u8_as_u16, Opcode::Dstore).parse(data),
            ...
            0x17 => map(u8_as_u16, Opcode::Fload).parse(data),
            ...
            0x38 => map(u8_as_u16, Opcode::Fstore).parse(data),
            ...
            0x84 => map((u8_as_u16, i8_as_i16), |(index, constant)| Opcode::Iinc(index, constant)).parse(data),
            0x15 => map(u8_as_u16, Opcode::Iload).parse(data),
            ...
            0x36 => map(u8_as_u16, Opcode::Istore).parse(data),
            ...
            0x16 => map(u8_as_u16, Opcode::Lload).parse(data),
            ...
            0x37 => map(u8_as_u16, Opcode::Lstore).parse(data),
            ...
            .parse(data),
            0xa9 => map(u8_as_u16, Opcode::Ret).parse(data),
            ...
            .parse(data),
            0xc4 => {
                // wide prefix: read the actual opcode and parse with u16 operands
                let (data, sub_opcode) = u8(data)?;
                match sub_opcode {
                    ...
                    _ => panic!("Unknown wide opcode: {:02x}", sub_opcode),
                }
            }
